...
export default class CartSidebar extends Cart {
  ...
  isCouponValid(codeToCheck, coupons) {
    ...
    const coupon = coupons.find(
      (c) => c.code.toLowerCase() === codeToCheck.toLowerCase()
    );
  ...
  checkCoupon() {
    ...
    const cartTotalWrapper = this.el.querySelector(
      ".cart-sidebar__total-amount"
    );
    const cartDiscountWrapper = this.el.querySelector(
    ...
    fetch(`${LOCATION_URL}/wp-json/wc/v3/coupons/`, {
      ...
      .then((data) => {
        ...
        if (couponRes) {
          statusDiv.textContent = `Kupon "${couponCode}" je prihvaćen: ${Math.round(
            ...
            couponRes.coupon.discount_type === "percent" ? "%" : " RSD"
          } popusta.`;
          ...
          cartTotalWrapper.classList.add(
            "cart-sidebar__total-amount--discounted"
          );
          cartDiscountWrapper.classList.remove(
            "cart-sidebar__discount-amount--hidden"
          );
      ...
      .catch((err) => {
        ...
        statusDiv.style.color = "red";
        validCoupon = null;
        cartDiscountWrapper.classList.add(
          "cart-sidebar__discount-amount--hidden"
        );
        cartTotalWrapper.classList.remove(
          "cart-sidebar__total-amount--discounted"
        );
      });
  }
